...
try:
    # 当作为包（例如 python -m lib.coding_agent）运行时，这个导入生效
    from .logger import logger, log_tool_call
    from .prompts import *
    from .tools import execute_tool

except (ImportError, SystemError):
    # 当直接运行文件（python lib/coding_agent_bak.py）时，相对导入会失败
    # 把当前文件所在目录加入 sys.path，然后用顶层导入降级处理
...
# --- Helpers to normalize OpenAI responses across providers/SDKs ---
def _get_total_tokens(response) -> int:
    """Safely extract total token usage from various response shapes."""
    try:
        usage = getattr(response, "usage", None)
        if usage is None:
            return 0
        # Object with attribute total_tokens
...
def _iter_response_parts(response):
    """Yield normalized parts as dicts for different response formats.

    Supports:
    - Responses API: response.output (with .to_dict())
    - Legacy Chat Completions: response.choices[0].message.content
    - Simple string responses.
    - Fallback to response.output_text if present.
    """
    # Responses API object
    if hasattr(response, "output") and response.output is not None:
        for part in response.output:
            # part may have to_dict()
    ...
    if hasattr(response, "output_text") and response.output_text:
        yield {"role": "assistant", "content": response.output_text}
        return
    ...
    if hasattr(response, "choices") and response.choices:
        try:
            ...
            # OpenAI SDK v1.0+ message struct
...
def coding_agent(
    ...
    while steps < max_steps:
        ...
        logger.info(f"call llm input:{clean_messages_for_llm(messages)}")
        ...
        usage = _get_total_tokens(response)
        has_function_call = False
        for part in _iter_response_parts(response):
            messages.append(part)

            yield part, messages, usage
            # If provider supports tool calls via Responses API
...
if __name__ == "__main__":
    ...
    client = OpenAI(api_key=_TUZI_API_KEY, base_url=_TUZI_BASE_URL)
    ...
    logger.info(f"call llm response:{response}")
